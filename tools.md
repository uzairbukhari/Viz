# The Tools Used in `Vizdum`

## Introduction

`Vizdum` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `Vizdum` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`Vizdum` uses git to manage its codebase. While in theory you don't have
to use Git once you download `Vizdum`, this project makes the assumption
...
While `Vizdum` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `Vizdum` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `Vizdum`; Grunt is
...
to get started with `Vizdum`. But for those curious or looking to go a
...
Grunt is the engine behind `Vizdum`. It's the magic that makes it move.
...
`Vizdum` comes with a `bower.json` file that looks something like this:
...
in `Vizdum` by default in there to see what I mean.
